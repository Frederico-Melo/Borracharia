...
export class DashboardService {
  ...
  get() {
    ...
    const stockByType = db.prepare(`SELECT type, COALESCE(SUM(quantity),0) AS quantity FROM products WHERE active=1 GROUP BY type`).all() as Array<{ type: 'TIRE' | 'WHEEL' | 'VALVE'; quantity: number }>;
    const stockSummary = { total: 0, tires: 0, wheels: 0, valves: 0 };
    for (const row of stockByType) {
      if (row.type === 'TIRE') stockSummary.tires = row.quantity;
      if (row.type === 'WHEEL') stockSummary.wheels = row.quantity;
      if (row.type === 'VALVE') stockSummary.valves = row.quantity;
      stockSummary.total += row.quantity;
    }
    ...
    return { todaySales: fromCents(today.sales), todayCount: today.count, monthSales: fromCents(currentMonth.sales), monthProfit: fromCents(currentMonth.profit), stockSummary, lowStock, recentSales };
  }
}

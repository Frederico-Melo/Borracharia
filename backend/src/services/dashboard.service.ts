...
export class DashboardService {
  ...
  get() {
    ...
    const stockByType = db.prepare(`SELECT type, COALESCE(SUM(quantity * sale_price_cents),0) AS value_cents FROM products WHERE active=1 GROUP BY type`).all() as Array<{ type: 'TIRE' | 'WHEEL' | 'VALVE'; value_cents: number }>;
    const stockValueCents = { total: 0, tires: 0, wheels: 0, valves: 0 };
    for (const row of stockByType) {
      if (row.type === 'TIRE') stockValueCents.tires = row.value_cents;
      if (row.type === 'WHEEL') stockValueCents.wheels = row.value_cents;
      if (row.type === 'VALVE') stockValueCents.valves = row.value_cents;
      stockValueCents.total += row.value_cents;
    }
    ...
    return { todaySales: fromCents(today.sales), todayCount: today.count, monthSales: fromCents(currentMonth.sales), monthProfit: fromCents(currentMonth.profit), stockValue, lowStock, recentSales };
  }
}

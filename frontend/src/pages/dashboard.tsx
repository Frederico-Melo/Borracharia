...
export function DashboardPage() {
  ...
  return <div><PageHeader title="Bom trabalho!" description="Acompanhe a operação da borracharia em tempo real." action={<Link className="button primary" to="/venda"><ShoppingBag size={18} /> Nova venda</Link>} />
    ...
    <section className="panel stock-summary"><div className="panel-heading"><div><h2>Resumo do estoque</h2><p>Quantidade de unidades disponíveis, separadas por categoria.</p></div><Link to="/estoque">Ver estoque <ArrowRight size={16} /></Link></div><div className="stock-summary-grid"><article className="stock-summary-card total"><small>Total geral</small><strong>{data.stockSummary.total} un.</strong><p>Todos os produtos disponíveis</p></article><article className="stock-summary-card"><small>Pneus</small><strong>{data.stockSummary.tires} un.</strong><p>Unidades em estoque</p></article><article className="stock-summary-card"><small>Rodas</small><strong>{data.stockSummary.wheels} un.</strong><p>Unidades em estoque</p></article><article className="stock-summary-card"><small>Bicos</small><strong>{data.stockSummary.valves} un.</strong><p>Unidades em estoque</p></article></div></section>
